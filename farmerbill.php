...
<input type="text" name="cost" value="<?php
    if ($row['milk_type'] == "cow||COW||Cow") {
        $priceQuery = "SELECT product_cost FROM Dairy_products WHERE product_id = 210";
    } else {
        $priceQuery = "SELECT product_cost FROM Dairy_products WHERE product_id = 209";
    }
    
    $priceResult = mysqli_query($conn, $priceQuery);
    if ($priceResult && $result = mysqli_fetch_array($priceResult)) {
...
<input type="text" name="amount" value="<?php
    $quantity = $row['sum(quan)'];
    $priceQuery = ($row['milk_type'] == "cow||COW||Cow") ? "SELECT product_cost FROM Dairy_products WHERE product_id = 210" : "SELECT product_cost FROM Dairy_products WHERE product_id = 209";
    
    $priceResult = mysqli_query($conn, $priceQuery);
    if ($priceResult && $result = mysqli_fetch_array($priceResult)) {
        $productCost = $result['product_cost'];
        echo $quantity * $productCost;
    } else {
        echo "Price not available";
    }
?>"/>
